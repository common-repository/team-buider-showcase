	...
	*/
	...
	public static function get_icon( $icon ) {
		...
		switch ( $icon ) {
			...
			break;
			default:
				$return = apply_filters( 'plwl_get_icon', '', $icon );
				return $return;
				break;
		}
	...
	public static function get_galleries() {
		...
		$galleries     = get_posts( array( 'post_type' => 'plwl-team', 'posts_per_page' => -1 ) );
		$gallery_array = array( 'none' => esc_html__( 'None', 'team-builder-showcase' ) );
		...
		$fancybox_options = array(
			...
			'i18n'            => array(
				'en' => array(
					...
					'NEXT'        => esc_html__( 'Next', 'team-builder-showcase' ),
					'PREV'        => esc_html__( 'Previous', 'team-builder-showcase' ),
					'Error'       => esc_html__( 'The requested content cannot be loaded. Please try again later.', 'team-builder-showcase' ),
					'PLAY_START'  => esc_html__( 'Start slideshow', 'team-builder-showcase' ),
					'PLAY_STOP'   => esc_html__( 'Pause slideshow', 'team-builder-showcase' ),
					'FULL_SCREEN' => esc_html__( 'Full screen', 'team-builder-showcase' ),
					'THUMBS'      => esc_html__( 'Thumbnails', 'team-builder-showcase' ),
					'DOWNLOAD'    => esc_html__( 'Download', 'team-builder-showcase' ),
					'SHARE'       => esc_html__( 'Share', 'team-builder-showcase' ),
					'ZOOM'        => esc_html__( 'Zoom', 'team-builder-showcase' ),
				)
			),
			'clickSlide'      => false,
			'clickOutside'    => false,
	...
	public static function get_image_sizes( $size_type = false, $size = '' ) {
		...
		$image_sizes_array['full']   = esc_html__( 'Full', 'team-builder-showcase' );
		$image_sizes_array['custom'] = esc_html__( 'Custom', 'team-builder-showcase' );
	 ...
	 * @since 2.3.3
	 */
	public static function plwl_gallery_placeholders() {

		return apply_filters( 'plwl_gallery_placeholders', array());
	 ...
	 * @since 2.3.3
	 */
	public static function plwl_placeholders_value($value,$object) {

		$values = apply_filters('plwl_placeholder_values',array('setting'=>array(),'front'=>array()),$object);
