...
class PlWl_Image {
	...
	public function get_image_size( $id, $type = 'creative-gallery', $sizes = array(), $crop = false ) {
		...
		if ( $image_full ) {
			...
			if( in_array( $type, $gallery_types ) ) {
				...
				$return = apply_filters( "plwl_resize_image_{$type}", $return, $id, $sizes, $crop );
			}

			return $return;

		}else{
			return new WP_Error( 'plwl-team-builder-showcase-error-no-url', esc_html__( 'No image with this ID.', 'team-builder-showcase' ) );
		}
    ...
    public function get_image_info( $args ) {
        ...
        if ( empty( $url ) ) {
            return new WP_Error( 'plwl-team-builder-showcase-error-no-url', esc_html__( 'No image URL specified for cropping.', 'team-builder-showcase' ) );
        }
        ...
        if ( ! file_exists( $file_path ) ) {
            return new WP_Error( 'plwl-team-builder-showcase-error-no-file', esc_html__( 'No file could be found for the image URL specified.', 'team-builder-showcase' ) );
        }
        ...
        if ( ! $size ) {
            return new WP_Error( 'plwl-team-builder-showcase-error-no-size', esc_html__( 'The dimensions of the original image could not be retrieved for cropping.', 'team-builder-showcase' ) );
        }
    ...
    public function resize_image( $url, $width = null, $height = null, $crop = false, $align = 'c', $quality = 100, $retina = false, $data = array(), $force_overwrite = false ) {
        ...
        // Filter args
        $args = apply_filters( 'plwl_gallery_resize_image_args', $args );
        ...
        if ( ! file_exists( $dest_file_name ) || ( file_exists( $dest_file_name ) && $force_overwrite ) ) {
            ...
            if ( is_wp_error( $editor ) ) {
                return new WP_Error( 'plwl-team-builder-showcase-error-no-editor', esc_html__( 'No image editor could be selected. Please verify with your webhost that you have either the GD or Imagick image library compiled with your PHP install on your server.', 'team-builder-showcase' ) );
            }
